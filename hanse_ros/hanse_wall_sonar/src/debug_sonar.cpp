#include "debug_sonar.h"
//! \todo{TODO dynamic reconfigure}


DebugSonarNode::DebugSonarNode(ros::NodeHandle n) : node(n){
...
void DebugSonarNode::sonar_laser_update(const sensor_msgs::LaserScan::ConstPtr& msg){
...
void DebugSonarNode::pos_update(const geometry_msgs::PoseStamped::ConstPtr &msg){
...
int main(int argc, char **argv)
{
    ...
    DebugSonarNode follow(n);

    //Subscribe to topic laser_scan (from sonar)
    ros::Subscriber sub_laser = n.subscribe<sensor_msgs::LaserScan>("/hanse/sonar/laser_scan", 1000, boost::bind(&DebugSonarNode::sonar_laser_update, &follow, _1));

    //Subscribe to the current position
    ros::Subscriber sub_pos = n.subscribe<geometry_msgs::PoseStamped>("/hanse/posemeter", 1000, boost::bind(&DebugSonarNode::pos_update, &follow, _1));
